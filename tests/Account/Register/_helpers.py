...
FIELD_RULES = [{'field': 'email_address',
...
def assert_not_redirected_to_login(page: Page) -> None:
    ...
    # /Account/Register 是后端匿名页；允许停留在 /Account/* 路由


def click_save(page: Page) -> None:
    # Register 的主按钮是 Register（不是 Save）
    btn = page.locator('role=button[name="Register"]').first
