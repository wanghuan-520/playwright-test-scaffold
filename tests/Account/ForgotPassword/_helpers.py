...
URL_PATH = '/Account/ForgotPassword'
...
def assert_not_redirected_to_login(page: Page) -> None:
    ...
    # /Account/ForgotPassword 是后端匿名页；允许停留在 /Account/* 路由


def click_save(page: Page) -> None:
    # ForgotPassword 的主按钮是 Submit（不是 Save）
    btn = page.locator('role=button[name="Submit"]').first
...
def detect_fatal_error_page(page: Page) -> str:
    ...
    # 标题命中（宽松）：只要含 Error 且不在正常 /Account/ForgotPassword
    try:
        title = (page.title() or "").strip()
    except Exception:
        title = ""
    if title and ("error" in title.lower()) and ("/Account/ForgotPassword" not in url):
...
ABP_MAX_LEN_EMAIL = 256  # email 最大长度
