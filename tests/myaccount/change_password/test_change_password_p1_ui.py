...
测试点（docs/requirements/account-profile-field-requirements.md）：
- P2 - UI 交互：
...
from tests.myaccount.change_password._helpers import step_shot
    ...
    with allure.step(f"[{field_name}] 点击显示/隐藏按钮"):
        try:
            ...
            if toggle_button is None:
                # 兜底：尝试找输入框所在容器中的按钮
                # 新 UI: 每个输入框后面都有一个按钮（包含 img）
                input_locator = auth_page.locator(input_selector)
                if input_locator.count() > 0:
                    # 找到输入框的父容器中的按钮
                    parent = input_locator.locator("xpath=ancestor::div[contains(@class, 'flex') or contains(@class, 'relative')][1]")
                    if parent.count() > 0:
