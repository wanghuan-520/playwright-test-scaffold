...
- API: POST /api/vibe/change-password
- 最小长度：6 字符
- 最大长度：无限制（MongoDB 不强制）
- 需要：小写字母、大写字母、数字、特殊字符
- 确认密码匹配：仅前端验证
...
class PasswordPolicy:
    """
    ABP Identity 密码策略数据类
    
    默认值（docs/requirements/account-profile-field-requirements.md）：
    - required_length: 6
    - require_digit: True
    - require_lowercase: True
    - require_uppercase: True
    - require_non_alphanumeric: True（特殊字符）
    - required_unique_chars: 1
    
    注意：最大长度无限制（MongoDB 不强制）
    """
    required_length: int
    require_digit: bool
