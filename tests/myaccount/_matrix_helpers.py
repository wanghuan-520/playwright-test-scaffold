...
def assert_frontend_has_error_evidence(page, selector: str, case_name: str) -> None:
  ...
  return { ariaInvalid, className, ariaDescribedBy, described, validationMessage: msg };
    ...
    # ✅ 检查 toast 错误消息（Sisyphus Research Platform 使用 toast 显示错误）
    if not ok:
        toast_selectors = [
            ...
            "[class*='toast']",
            "[class*='notification']",
            "li:has(button:has-text('Dismiss'))",  # Sisyphus toast pattern
        ]
        for ts in toast_selectors:
            try:
                toast_elem = page.locator(ts)
                if toast_elem.count() > 0 and toast_elem.first.is_visible(timeout=500):
                    toast_text = toast_elem.first.inner_text(timeout=500)
                    if toast_text and len(toast_text.strip()) > 0:
                        evidence["toast_text"] = toast_text
                        ok = True
                        break
            except Exception:
                continue
