...
def abp_profile_put(page: Page, baseline: dict, patch: dict) -> dict:
    ...
    注意：测试代码使用 firstName/lastName，但 API 使用 name/surname
    此函数会自动转换字段名。
    ...
    # 转换字段名：firstName/lastName -> name/surname
    payload = {}
    combined = dict(baseline or {})
    combined.update(patch or {})
    ...
    for test_field, api_field in field_map.items():
        if test_field in combined:
            payload[api_field] = combined[test_field]
    
    try:
        resp = page.request.put(f"{origin}/api/vibe/my-profile", data=payload)
        body = ""
        try:
            body = resp.text() or ""
        except Exception:
