...
DESC_REQUIRED_EMPTY_KNOWN_BUG = (
    ...
    "Baseline：UserName=qatest_{uuid}, Email=qatest_{uuid}@testmail.com, Password=ValidPass123!；随后清空一个字段提交。\n"
...
DESC_EMAIL_BOUNDARY = (
    "边界值三点采样：max-1/max/max+1（255/256/257）。\n"
    "Baseline：UserName=qatest_{uuid}, Email=qatest_{uuid}@testmail.com, Password=ValidPass123!。\n"
    "目标输入：suffix=uuid[:10]；255/256 用 '@t.co'（短域名）；257 会被 email 总长度限制。\n"
    "期望：255/256 允许提交（无 error UI；若抓到响应则非 4xx/5xx）；257 必须前端截断或后端拒绝。"
)
DESC_PASSWORD_BOUNDARY = (
...
def ensure_register_page(page: Page, po) -> None:
    try:
        # 使用新的selector，但保持向后兼容
        selector = getattr(po, 'USERNAME_INPUT', None) or getattr(po, 'USERNAME_INPUT', None)
...
def fill_baseline(po, *, suffix: str) -> None:
    ...
    po.fill_email(f"qatest_{suffix}@testmail.com")
