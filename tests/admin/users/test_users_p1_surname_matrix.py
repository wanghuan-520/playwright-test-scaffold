"""
Admin Users - P1 Surname Validation Matrix
...
def _scenarios():
    ...
    return [
        ...
        ("chinese", "李", True, "格式：中文", False),
        ("special", "O'Brien", True, "格式：特殊字符", False),
        ("numbers", "Surname123", True, "格式：数字", False),
        
        # 长度（3）
        ("len_1", "S", True, "长度：1", False),
        ("len_64", "S" * max_len, True, "长度：64（最大）", False),
        ("len_65", "S" * (max_len + 1), False, "长度：65（超长）", False),
...
@pytest.mark.matrix
@allure.feature("用户管理")
@allure.story("Surname 矩阵")
@pytest.mark.parametrize(
    "name,value,should_save,note,require_fe_error",
    [pytest.param(*s, id=s[0]) for s in _scenarios()],
)
def test_p1_users_surname_matrix(admin_page, name, value, should_save, note, require_fe_error):
    """10 场景：可选/格式/长度/安全"""
    suffix = now_suffix()
    ...
    scenario = UsersMatrixScenario(
        case_name=f"surname_{name}",
        selector="input[placeholder='Surname']",
        patch={"username": f"user{suffix}", "email": f"test{suffix}@test.com", "password": "Test@123456", "surname": s},
