...
from pages.admin_users_roles_page import AdminUsersRolesPage
from tests.admin.users_roles._helpers import assert_not_redirected_to_login
from utils.logger import TestLogger

logger = TestLogger('users_roles' + "_p2")


@pytest.mark.P2
@pytest.mark.ui
@allure.feature('AdminUsersRoles')
@allure.story("P2")
...
def test_p2_fields_visible(auth_page: Page):
    ...
    selectors = ['role=textbox[name="Search..."]']
    if not selectors:
    ...
    po.take_screenshot('users_roles' + "_p2_fields_visible", full_page=True)
...
def test_p2_keyboard_tab_navigation(auth_page: Page):
    ...
    po.take_screenshot('users_roles' + "_p2_keyboard_tab")
    logger.end(success=True)
