# Admin Settings Feature Management UI 自动化测试计划

## 0. 生成信息（用于可追溯）
- **URL**: `https://localhost:3000/admin/settings/feature-management`
- **slug**: `admin_settings_feature_management`
- **生成时间**: 2025-12-24 23:34:20
- **是否需要登录态**: 是
- **证据链目录**: `docs/test-plans/artifacts/admin_settings_feature_management/`
...
| button | Manage Host Features | 按钮操作 | role/name | `page.get_by_role("button", name="Manage Host Features")` | 可点击 / loading/禁用态 / 触发结果（toast/错误提示/跳转） | 需要登录态 |
...
# ============================================================
# 页面对象：AdminSettingsFeatureManagementPage
# - 目标：封装稳定定位器与业务操作
# - 原则：短小、直白、少分支
# ============================================================
class AdminSettingsFeatureManagementPage(BasePage):
    ...
    def navigate(self) -> None:
        self.goto("https://localhost:3000/admin/settings/feature-management")
...
## 5. 自动化实现建议（对齐本仓库）
### 5.1 页面类实现
- 建议 PageObject：`pages/admin_settings_feature_management_page.py`（类名 `AdminSettingsFeatureManagementPage`，继承 `core/base_page.py:BasePage`）
- 把“业务动作”封装成方法（不要在测试里散落 click/fill）
- 定位器优先级：role/name、label、data-testid；必要时提供 CSS 兜底（两套策略）

### 5.2 测试类实现
- 建议测试目录：`tests/admin/admin_settings_feature_management/` 或对齐现有 suite 目录
