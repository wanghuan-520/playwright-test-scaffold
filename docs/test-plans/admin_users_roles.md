# Admin Users Roles UI 自动化测试计划

## 0. 生成信息（用于可追溯）
- **URL**: `https://localhost:3000/admin/users/roles`
- **slug**: `admin_users_roles`
- **生成时间**: 2025-12-24 23:34:23
- **是否需要登录态**: 是
- **证据链目录**: `docs/test-plans/artifacts/admin_users_roles/`
...
|---------|----------|----------|----------|--------|----------|----------|
| input | Search... | 表单字段/交互 | css | `page.locator("input.file:text-foreground")` | 可输入 / 可清空 / 校验提示（必填/格式/长度） | 需要登录态 |
...
class AdminUsersRolesPage(BasePage):
    ...
    def navigate(self) -> None:
        self.goto("https://localhost:3000/admin/users/roles")
...
## 5. 自动化实现建议（对齐本仓库）
### 5.1 页面类实现
- 建议 PageObject：`pages/admin_users_roles_page.py`（类名 `AdminUsersRolesPage`，继承 `core/base_page.py:BasePage`）
- 把“业务动作”封装成方法（不要在测试里散落 click/fill）
- 定位器优先级：role/name、label、data-testid；必要时提供 CSS 兜底（两套策略）

### 5.2 测试类实现
- 建议测试目录：`tests/admin/admin_users_roles/` 或对齐现有 suite 目录
