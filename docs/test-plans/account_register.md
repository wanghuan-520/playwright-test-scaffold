...
|---------|----------|----------|----------|--------|----------|----------|
| input | Input.UserName | 表单字段/交互 | css | `page.locator("#Input_UserName")` | 可输入 / 可清空 / 校验提示（必填/格式/长度） | 无 |
| input | Input.EmailAddress | 表单字段/交互 | css | `page.locator("#Input_EmailAddress")` | 可输入 / 可清空 / 校验提示（必填/格式/长度） | 无 |
| input | Input.Password | 密码输入 | css | `page.locator("#Input_Password")` | 可输入 / 可清空 / 校验提示（必填/格式/长度） / mask 显示/不回显明文 | 无 |
| button | English | 按钮操作 | role/name | `page.get_by_role("button", name="English")` | 可点击 / loading/禁用态 / 触发结果（toast/错误提示/跳转） | 无 |
| button | Register | 按钮操作 | role/name | `page.get_by_role("button", name="Register")` | 可点击 / loading/禁用态 / 触发结果（toast/错误提示/跳转） | 无 |
...
class AccountRegisterPage(BasePage):
    ...
    def navigate(self) -> None:
        self.goto("https://localhost:44320/Account/Register")
...
### 5.2 测试类实现
- 建议测试目录：`tests/account/account_register/` 或对齐现有 suite 目录
