...
### 🚀 快速开始（新手必看）
- **[QUICKSTART.md](../QUICKSTART.md)** ⚡⚡⚡ - 3 步生成测试（最简单！）
- **[快速模板](./quick-templates.md)** ⚡ - 一键生成测试（开箱即用）

### 核心指南
- **[未知页面完整测试流程](./unknown-page-complete-workflow.md)** ⭐ - 从零到完整测试的终极工作流（推荐）
- **[Spec-Kit 实战落地手册](./spec-kit-hands-on-guide.md)** - 已知页面的测试流程（实战）
- **[Spec-Kit 快速入门](./spec-kit-quickstart.md)** - Spec-Kit 实战操作指南（实操）
- **[Spec-Kit 使用指南](./spec-kit-guide.md)** - AI 驱动的规约→代码工作流（理论）
- **[框架概览](./framework_overview.md)** - pytest + Playwright 运行链路
- **[宪法深度解读](./constitution-deep-dive.md)** - 项目哲学与架构原则深度剖析

### 运维规范
- **[测试报告管理](./test-reports-management.md)** - Allure 报告文件夹组织规范

### 测试报告
- **[Admin Users 测试报告](./admin-users-final-test-report.md)** 📊 - 用户管理测试最终报告（2026-01-05）
- **[错误验证修复总结](./error-validation-fix-summary.md)** - 验证测试修复详情
- **[错误验证诊断](./error-validation-diagnosis.md)** - 错误提示问题诊断
- **[定位器修复总结](./admin-users-locator-fixes.md)** - Playwright 定位器修复详情

### 需求文档
- **[需求文档](./requirements/)** - 功能需求和验收标准
...
- 如果你想先快速理解“pytest 入口 → fixtures → 账号池/登录态 → 失败证据链 → Allure 汇总”的完整运行链路，建议先读：`docs/framework_overview.md`。
...
│   │       ├── change_password/             # 修改密码（套件：P0/P1/mutate）
...
│
├── docs/                         # 文档
│   ├── architecture.md           # 本文档 - 架构设计总览
│   ├── framework_overview.md     # 框架概览
│   ├── spec-kit-guide.md         # Spec-Kit 理论指南
│   ├── spec-kit-quickstart.md    # Spec-Kit 快速入门
│   ├── constitution-deep-dive.md # 宪法深度解读
│   ├── test-reports-management.md # 测试报告管理规范
│   ├── rules.yaml                # 可执行规则（可选：缺失时 `utils/rules_engine.py` 会使用默认值）
│   ├── requirements/             # 需求文档
│   └── test-plans/               # 生成的测试计划
│
├── specs/                        # （可选）规格层：spec/plan/tasks（Spec-Driven）
│   └── <NNN>-<slug>/             # 单个页面/特性的规格目录
│
├── scripts/                      # 工具脚本
│   ├── speckit.py                # Spec-Kit 工作流入口
│   ├── speckit_core.py           # Spec-Kit 核心逻辑
│   ├── cleanup_allure_folders.sh # 清理混乱的测试报告文件夹
│   └── register_*.py             # 测试数据注册脚本
│
├── reports/                      # pytest 日志
├── screenshots/                  # 测试截图
├── allure-results/               # Allure 原始结果（pytest 输出）
├── allure-report/                # Allure 生成的 HTML 报告
└── .allure-cache/                # Allure 历史缓存（跨运行的趋势数据）
#
# 测试报告结构（标准化，禁止创建临时文件夹）：
# - `allure-results/`：pytest --alluredir 输出
# - `allure-report/`：allure generate 输出
# - `.allure-cache/`：历史趋势数据缓存
# 
# ⚠️ 禁止：allure-report-*/、allure-results-* 等临时文件夹
# 详见：docs/test-reports-management.md
```
...
## 可选工作流：Spec-Driven（spec-kit 风格）

目标：把一次测试建设从“只剩代码”变成“规格可审阅、计划可追溯、任务可执行”。
...
说明：
- `docs/test-plans/<slug>.md` 是生成器契约输入（`generators/plan_to_automation.py` 会解析 URL/页面类型/登录态）
- `docs/test-plans/artifacts/<slug>/metadata.json` 可选，但建议补齐 `locator_hints`（定位器证据链）
...
| 日期 | 变更 |
|------|------|
| 2026-01-05 | **测试报告优化**: 统一测试报告文件夹结构，禁止临时文件夹，新增管理规范文档 |
| 2026-01-05 | **文档体系完善**: 新增 Spec-Kit 快速入门、宪法深度解读、测试报告管理规范 |
| 2026-01-04 | **Spec-Kit 本地化**: 完整中文化 Spec-Kit 框架（15 个文件：命令 + 模板 + 宪法）|
...
| 2025-12-23 | 新增 Change Password 测试计划与证据链（docs/test-plans + artifacts） |
| 2025-12-24 | 新增 Change Password 套件代码落盘（pages + tests + test-data），并发可跑（xdist） |
